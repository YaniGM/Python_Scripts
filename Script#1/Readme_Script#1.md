...
Cabe destacar que la wordlist resultante puede llegar a ser un archivo con cientos de miles, e incluso millones de combinaciones por ser un producto cartesiano entre los elemntos de cada archivo. Por lo que un ataque de fuerza bruta, por ejemplo con ``hydra``, en un archivo de un elevado número de combinaciones puede consumir mucho tiempo y recursos de la máquina host. Sin embargo, para archivos de relativamente pocos de elementos, esta solución se presenta como ideal. Incluso para archivos con un número muy grande de elementos, ya que se pueden ir troceando y crear combinaciones más razonablemente pequeñas.

Desde el directorio de descarga en una shell de Linux:
```bash
